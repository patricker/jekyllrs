...
# Large site benchmark: generates a realistic site (~500 pages) with posts,
# collections, includes, layouts, and Liquid loops, then times site.process.
#
# Usage:
#   ruby benchmark/large-site.rb
#
# Set JEKYLL_RUST_LIB to benchmark the Rust engine.
...
def run_benchmark
  Dir.mktmpdir("jekyll-bench") do |dir|
    ...
    if File.exist?(BASELINE_FILE)
      ...
      if baseline[engine]
        prev = baseline[engine]["best"]
        delta = ((min - prev) / prev * 100).round(1)
        indicator = delta < 0 ? "faster" : "slower"
        puts "  vs baseline: #{delta.abs}% #{indicator} (was #{format('%.3f', prev)}s)"
      end
    end

    # Save new baseline
    baseline = File.exist?(BASELINE_FILE) ? JSON.parse(File.read(BASELINE_FILE)) : {}
    baseline[engine] = {
      "best"    => min,
      "average" => avg,
      "runs"    => times,
